 ...
 */
OneButton::OneButton(PORT_t*const port, const int8_t pin, const bool activeLow, const bool pullupActive) :
    _port{port},
    _pin{pin},
    _buttonPressed{activeLow ? 0 : 1},
...
void OneButton::init() {
  if (_pullupActive) {
    ...
    _port->DIRCLR = static_cast<uint8_t> ( 1 << _pin );
    (&_port->PIN0CTRL)[_pin] |= PORT_PULLUPEN_bm;
  } else {
...
void OneButton::setDebounceTicks(const unsigned int ticks)
{
...
void OneButton::setClickTicks(const unsigned int ticks)
{
  _clickTicks = ticks;
} // setClickTicks
...
void OneButton::setPressTicks(const unsigned int ticks)
{
  _pressTicks = ticks;
} // setPressTicks
...
{
  if (_pin >= 0) {
    bool digitalRead = static_cast<bool> ( _port->IN & (1 << _pin) );
    tick(digitalRead == _buttonPressed);
  }
}
